...
class ConfusionMatrix:
    ...
    def compute(self):
        ...
        # F1评分（F1） = 2 * (Precision * Recall) / (Precision + Recall)
        precision = TP / (TP + FP + EPSILON)
        recall = sensitivity  # Recall 与 Sensitivity 相同
        f1_score = 2 * (precision * recall) / (precision + recall + EPSILON)

        # 准确性（ACC） = (TP + TN) / (TP + TN + FP + FN)
        accuracy = (TP + TN) / (TP + TN + FP + FN + EPSILON)

        return acc_global, acc, iu, sensitivity, specificity, f1_score, accuracy
    ...
    def __str__(self):
        acc_global, acc, iu, sensitivity, specificity, f1_score, accuracy = self.compute()
        return (
            'global correct: {:.1f}\n'
            'average row correct: {}\n'
            'IoU: {}\n'
            ...
            'ACC:{}').format(
                ...
                iu.mean().item() * 100,
                [f'{x * 100:.1f}' for x in sensitivity],
                [f'{x * 100:.1f}' for x in specificity],
                [f'{x * 100:.1f}' for x in f1_score],
                [f'{x * 100:.1f}' for x in accuracy])
